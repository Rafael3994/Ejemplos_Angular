...
// va a servir para definir las opctiones que debe tener una ruta
 ...
 * 
 * LA IDEA ES QUE A TRAVÉS DE UN EVENTO DEL USUARIO EN EL DOM (CLIK, HOVER, LONG PRESS...)
 * SE INICIE UNA PRECARGA O NO DE MÓDULOS. POR LO QUE COSEGUIRÍAMOS ADELANTARNOS AL USUARIO
 ...
 * ? POR EJEMPLO: 
 * * SI UN USUARIO PASA EL CURSOR POR UN ELEMENTO DEL MENÚ AL QUE CLARAMENTE VA A NAVEGAR
...
export class PreloadingService {
  ...
  // a quien esté subscrito al mismo a través del metodó  .next(newValue)
  private _subject = new Subject<PreloadingOptions>();

  // Cualquier Subject puede ser tratado como un Observable y es el que tenemos
  // que hacer público.
  // Con él vamos a ofrecer la opciones de la ruta que desea ser precargada como un Observable
  ...
  comenzarPrecarga(routePath: string) {
    // Cremoas unas opciones de precarga
    const opcionesPrecarga: PreloadingOptions = new PreloadingOptions(routePath, true);

    // Emitimos las opciones que desean se precargadas.
